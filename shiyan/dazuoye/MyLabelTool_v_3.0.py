...
class LabelImage:
    ...
    def text_ui(self):
        ...
        self.text.bind('<Control-Shift-KeyPress-S>', self.save_and_exit)  # 按Ctrl+Shift+S键保存并退出
...
class CreateUIAndShowImg(tk.Tk):
    def __init__(self):
        ...
        self.flag = False  # json文件保存标志，初始为False
        # 显示界面
        self.setup_ui()
    ...
    def select_rect_on_listbox(self, event):
        self.be_selected_rect = list(self.listbox.curselection())
        if len(self.be_selected_rect) > 0:
            ...
            for i in self.be_selected_rect:
                ...
                self.text1.set(str(rect_pos))  # 显示对应的位置坐标

                key_pos = "position" + str(i)  # 显示是否保存有标注信息
                if self.mess_list:
                    ind = []
                    for j in range(len(self.mess_list)):  # 遍历每个字典
                        if key_pos in self.mess_list[j]:  # 如果这个键在字典中
                            ind.append(j)
                    if len(ind) > 0:
                        self.text2.set('\n\t该位置已标注信息！\n'
                                       + '\n\t点击此处查看或修改。')
        ...
        if len(self.be_selected_rect) > 0:
            for i in range(len(self.img_rect)):
                if i in self.be_selected_rect:
                    if self.mess_list:  # 如果该矩形保存了标注信息，则删除对应的信息
                        key_pos = "position" + str(i)  # 这个矩形保存下来对应的键
                        # 矩形位置和标注信息是字典存储在self.mess_list列表中，
                        # 找到矩形位置对应的键，从列表中移除就将对应的信息也删除了
                        ind = []
                        for j in range(len(self.mess_list)):  # 遍历每个字典
                            if key_pos in self.mess_list[j]:  # 如果这个键在字典中
                                ind.append(j)
                    ...
                    self.be_selected_rect.remove(i)  # 从ListBox中删除该矩形
                    self.img_rect.pop(i)  # 从列表中中删除该矩形
                    self.text2.set('\n\t该标注区域位置和标注信息已删除！')
            self.show_listbox()
            self.show_and_draw_img()
            self.text1.set('未选择标注区域！')
        ...
        if len(self.be_selected_rect) > 0:
            if self.label_img is None:  # 确保仅点击了一次
                ...
                for i in self.be_selected_rect:
                    ...
                    # 所要标注的区域是否已经标注了，显示已标注的内容
                    key_con = "content" + str(i)
                    if self.mess_list:
                        ind = []
                        for j in range(len(self.mess_list)):  # 遍历每个字典
                            if key_con in self.mess_list[j]:  # 如果这个键在字典中
                                ind.append(j)
                        if len(ind) > 0:
                            self.content = self.mess_list[ind[-1]][key_con]
                # 否则显示内容为最近一次显示的内容
                ...
                if self.mess_list:  # 不为空
                    ...
                    for j in range(len(self.mess_list)):
                        if key_pos in self.mess_list[j]:  # 如果键在字典中
                            ind.append(j)
                    for k in ind:
                        self.mess_list.pop(ind[-1])  # 从列表中删除有key_pos键的这个字典
    ...
    # 保存为json文件
    def save_json(self):
        if len(self.message_dict) > 0 and self.flag is False:
    ...
    def setup_ui(self):
        ...
        label_4.bind('<Control-Button-3>', self.write_json)  # Ctrl+右击保存标注信息
        ...
        self.button4.place(x=self.but_win_w / 2 + 10, y=35 + self.but_height_2,
                           width=self.but_width_2, height=self.but_height_2)
        self.button5 = tk.Button(but_win, text='保存为JSON文件', bg=self.but_bg_2,
                                 activebackground=self.but_active_bg_2,
